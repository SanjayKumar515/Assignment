...
export const UserService = {
  loginUser: async (payload: any) => {
    ...
    return APIKit.post("auth/login", payload, apiHeaders);
  },
  ...
  refreshToken: async (token: string) => {
    ...
    try {
      const response = await fetch(
        "https://testlink4.pillersofttechnologies.com/api/auth/refresh",
        requestOptions,
      );
  ...
  logoutUser: async (token: string) => {
    ...
    try {
      const response = await fetch(
        "https://testlink4.pillersofttechnologies.com/api/auth/logout",
        requestOptions,
      );
